...
local IsQuestFlaggedCompleted = IsQuestFlaggedCompleted
		or (C_QuestLog and C_QuestLog.IsQuestFlaggedCompleted)
		or function(questID)
			local completed = GetQuestsCompleted and GetQuestsCompleted() or {};
			return tContains(completed, questID);
		end;
...
NIT.lastInstanceReset = 0;
NIT.lastInstanceResetName = nil;
function NIT:markInstanceReset(instanceName)
	NIT.lastInstanceReset = GetServerTime();
	NIT.lastInstanceResetName = instanceName;
	local previous = NIT.data and NIT.data.instances and NIT.data.instances[1];
	if (previous and (not instanceName or previous.instanceName == instanceName)) then
		previous.resetTime = NIT.lastInstanceReset;
	end
end

--Sirus uses the original 3.3.5 hexadecimal unit GUID format.
function NIT:getCreatureGUIDInfo(GUID)
	if (not GUID) then
		return;
	end
	if (string.sub(GUID, 1, 6) == "0xF130") then
		return "Creature", tonumber(string.sub(GUID, 9, 12), 16), nil;
	end
	local unitType, _, _, _, zoneID, npcID = strsplit("-", GUID);
	return unitType, tonumber(npcID), tonumber(zoneID);
end

function NIT:combatLogEventUnfiltered(...)
	local timestamp, subEvent, destGUID;
	if (CombatLogGetCurrentEventInfo) then
		local hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destName, destFlags, destRaidFlags, spellID, spellName;
	...
	--elseif (subEvent == "UNIT_DIED" and UnitLevel("player") == NIT.maxLevel and string.match(destGUID, "Creature")) then
	local unitType, npcID = NIT:getCreatureGUIDInfo(destGUID);
	if (subEvent == "UNIT_DIED" and unitType == "Creature" and npcID) then
...
instanceDetectionFrame:SetScript('OnEvent', function(self, event, ...)
	local subEvent, sourceGUID, destGUID;
	if (CombatLogGetCurrentEventInfo) then
		local _;
		_, subEvent, _, sourceGUID, _, _, _, destGUID = CombatLogGetCurrentEventInfo();
	else
		--WoW 3.3.5 passes combat log data directly to the event handler.
		local _;
		_, subEvent, sourceGUID, _, _, destGUID = ...;
	end
	if (subEvent == "SWING_DAMAGE" or subEvent == "SPELL_DAMAGE" or subEvent == "RANGE_DAMAGE") then
		local sourceType = NIT:getCreatureGUIDInfo(sourceGUID);
		local destType = NIT:getCreatureGUIDInfo(destGUID);
...
function NIT:enteredInstance(isReload, isLogon, checkAgain)
	...
	--Sirus 3.3.5 returns 1 here instead of the boolean true.
	if (instance and ((instanceType == "party") or (instanceType == "raid")
		...
		if (isGhost) then
			...
			isGhost = false;
			NIT:trimDatabase();
			--Sirus can omit instanceID; use the localized instance name as a stable fallback key.
			if (createdInstance) then
				NIT:addInstanceCount(instanceID or instanceName);
			end
			local type = "unknown";
...
function NIT:parseGUID(unit, GUID, source)
	...
	if (GUID and doGUID and NIT.inInstance and (not string.match(source, "combatlog") or GetServerTime() - NIT.inInstance > 2)) then
		local unitType, npcID, zoneID = NIT:getCreatureGUIDInfo(GUID);
...
function NIT:mergeLastInstances(GUID, source)
	...
	if (data and (data.instanceID or data.instanceName)) then
		NIT:removeInstanceCount(data.instanceID or data.instanceName);
	end
	if (NIT.data.instances[1]) then
...
function NIT:recordGroupInfo()
	if (not NIT.inInstance or not NIT.data.instances[1]) then
		return;
	end
...
function NIT:deleteInstance(num, displayNum)
	...
	if (data) then
		...
		if (data.instanceID or data.instanceName) then
			NIT:removeInstanceCount(data.instanceID or data.instanceName);
		end
		if (displayNum) then
...
function NIT:updateWeeklyResetTime()
	...
	elseif (not NIT.db.global[NIT.realm].weeklyResetTime or NIT.db.global[NIT.realm].weeklyResetTime < GetServerTime()) then
		NIT.db.global[NIT.realm].weeklyResetTime = GetServerTime() + 604800;
	end
end
...
function NIT:recordCooldowns()
	...
	if (numTradeSkills < 1) then
		...
		numTradeSkills = GetNumCrafts and GetNumCrafts() or 0;
	end
	if (duplicateCheck) then
		for i = 1, numTradeSkills do
			local secondsLeft = (GetTradeSkillCooldown and GetTradeSkillCooldown(i))
					or (GetCraftCooldown and GetCraftCooldown(i));
			if (secondsLeft and secondsLeft > 60) then
				local skillName = (GetTradeSkillInfo and GetTradeSkillInfo(i))
						or (GetCraftInfo and GetCraftInfo(i));
	...
	for bag = 0, NUM_BAG_SLOTS do
		for slot = 1, GetContainerNumSlots(bag) do
			...
			local itemID = GetContainerItemID and GetContainerItemID(bag, slot)
					or tonumber((itemLink or ""):match("item:(%d+)"));
			if (itemID) then
				local itemName = GetItemInfo(itemID);
