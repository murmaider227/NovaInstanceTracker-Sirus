...
function NIT:createLevelLogFrame()
	local frame = CreateFrame("Frame", "NITLevelLogFrame", UIParent, BackdropTemplateMixin and "BackdropTemplate" or nil);
	...
	frame.topFrame = CreateFrame("Frame", "$parentTopFrame", frame, BackdropTemplateMixin and "BackdropTemplate" or nil);
	...
	if (frame.close:GetDisabledTexture()) then
	...
	frame.createSimpleLineFrame = function(count, data)
		if (not frame.simpleLineFrames[count]) then
			...
			obj.borderFrame = CreateFrame("Frame", "$parentBorderFrame", obj, BackdropTemplateMixin and "BackdropTemplate" or nil);
