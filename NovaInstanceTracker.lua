...
-- Sirus backports C_Timer as method calls, while modern Classic uses dot calls.
-- Keep the adapter local to NIT so other Sirus addons retain their native API.
local sirusTimer = _G.C_Timer;
local C_Timer = {
	After = function(duration, callback)
		return sirusTimer:After(duration, callback);
	end,
	NewTimer = function(duration, callback)
		return sirusTimer:NewTimer(duration, callback);
	end,
	NewTicker = function(duration, callback, iterations)
		return sirusTimer:NewTicker(duration, callback, iterations);
	end,
};
...
NIT.dragonLib = {
	GetPlayerZonePosition = function()
		if (SetMapToCurrentZone) then
			SetMapToCurrentZone();
		end
		local x, y = GetPlayerMapPosition("player");
		local zone = GetCurrentMapAreaID and GetCurrentMapAreaID();
		return x, y, zone;
	end,
	GetLocalizedMap = function()
		return GetZoneText();
	end,
};
NIT.dragonLibPins = {};
...
NIT.maxLevel = GetMaxPlayerLevel and GetMaxPlayerLevel() or 80;
...
f:SetScript('OnEvent', function(self, event, msg)
	...
	if (success) then
		if (NIT.markInstanceReset) then
			NIT:markInstanceReset(instance);
		end
...
function NIT:createBroker()
	...
	local frame;
	if (NIT.LDBIcon.GetMinimapButton) then
		frame = NIT.LDBIcon:GetMinimapButton("NovaInstanceTracker");
	elseif (NIT.LDBIcon.objects) then
		frame = NIT.LDBIcon.objects["NovaInstanceTracker"];
	end
	if (frame) then
...
function NIT:updateMinimapButton(tooltip, frame)
	...
	if (NIT.inInstance) then
		...
		tooltip:AddLine(" ");
		local separatorAnchor = _G[tooltip:GetName() .. "TextLeft" .. tooltip:NumLines()];
		tooltip.NITSeparator:SetPoint("TOP", separatorAnchor or tooltip, "CENTER");
		...
		tooltip:AddLine(" ");
		local separatorAnchor2 = _G[tooltip:GetName() .. "TextLeft" .. tooltip:NumLines()];
		tooltip.NITSeparator2:SetPoint("TOP", separatorAnchor2 or tooltip, "CENTER");
...
if (NITInstanceFrameClose:GetDisabledTexture()) then
...
if (NITInstanceDCFrameClose:GetDisabledTexture()) then
...
if (NITTradeLogFrameClose:GetDisabledTexture()) then
...
if (NITTradeCopyFrameClose:GetDisabledTexture()) then
...
if (NITAltsFrameClose:GetDisabledTexture()) then
...
f:SetScript('OnEvent', function(self, event, ...)
	if (event == "GOSSIP_SHOW") then
		...
		if (npcID == "20201" and NIT.db.global.autoBlackMorass and IsQuestFlaggedCompleted(10298)) then
			--Sa'at													--Hero of the Brood
			NIT:selectGossipOption(1);
			return;
		end
		if (npcID == "20142" and NIT.db.global.autoCavernsFlight and IsQuestFlaggedCompleted(10279)) then
...
function NIT:createSimpleTextFrame(name, width, height, x, y, borderSpacing)
	...
	if (borderSpacing) then
		frame.borderFrame = CreateFrame("Frame", "$parentBorderFrame", frame, BackdropTemplateMixin and "BackdropTemplate" or nil);
...
if (NIT.isWrath and LFGDungeonReadyDialog and LFGDungeonReadyDialogRoleIcon) then
